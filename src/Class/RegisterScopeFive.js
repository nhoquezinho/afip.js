...
module.exports = class RegisterScopeFive extends AfipWebService {
    ...
    async getTaxpayerDetails(identifier, tokenAndSign) {
        // Get token and sign
        let { token, sign } = tokenAndSign ? tokenAndSign : await this.afip.GetServiceTA("ws_sr_padron_a5");
     ...
     **/
    async getTaxpayersDetails(identifiers, tokenAndSign) {
        // Get token and sign
        let { token, sign } = tokenAndSign ? tokenAndSign : await this.afip.GetServiceTA("ws_sr_padron_a5");
        ...
        return this.executeRequest("getPersonaList_v2", params).then((res) => res.persona);
    }
    ...
    async executeRequest(operation, params = {}) {
        ...
        return results[operation === "getPersona_v2" ? "personaReturn" : operation === "getPersonaList_v2" ? "personaListReturn" : "return"];
    }
};
